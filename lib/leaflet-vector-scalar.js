...
L.ScalarLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  _initScalar: function _initScalar(self) {
    ...
    this._map.on("movestart", self._hideScalar); //暂时关闭
...
var Scalar = function Scalar(params) {
  ...
  var map_w = params.map.getSize().x;
  var map_h = params.map.getSize().y;
  var view = {
    width: map_w,//document.documentElement.clientWidth,
    height: map_h //map_scalle?document.documentElement.clientHeight
  };
  var OVERLAY_ALPHA = Math.floor(params.opacity * 255);
  ...
  var interpolateField = function interpolateField(grid, bounds, extent, callback) {
    ...
    (function batchInterpolate() {
      // animationLoop = requestAnimationFrame(batchInterpolate);
      var start = Date.now();
  ...
  var getBase64Image = function getBase64Image(data) {
    var canvas = document.createElement('canvas');
  ...
  var drawOverlay = function drawOverlay(field) {
    ...
    if (params.geojson){
      ...
      // 应用裁切
      ctx.beginPath();
      params.geojson.geometry.coordinates[0].forEach((coord, index) => {
          var coords = L.latLng(coord[1],coord[0]);
          var ll=params.map.latLngToContainerPoint(coords);
          if (index === 0) {
              ctx.moveTo(ll.x, ll.y);
          } else {
              ctx.lineTo(ll.x, ll.y);
          }
          console.log(ll.x, ll.y);
      });
      // ctx.closePath();
      ctx.clip();

      // ctx.putImageData(field.imageData, 0, 0);
      var base64 = getBase64Image(field.imageData)
      var img = new Image();
      img.src = base64;
      img.onload = function() {
